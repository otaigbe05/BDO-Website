...
export const CartProvider = ({ children }) => {
  ...
  const getCartTotal = useCallback(() => {
    ...
    const total = cartItems.reduce((sum, item) => {
      const price = item.variant?.sale_price_in_cents ?? item.variant?.price_in_cents ?? (item.variant?.price ? item.variant.price * 100 : 2900);
      return sum + (price * item.quantity);
    }, 0);
    
    // Fallback if currency_info is missing from variant
    try {
